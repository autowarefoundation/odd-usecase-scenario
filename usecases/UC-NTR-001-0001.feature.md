# Feature: Wait for an overtake while stationary

Ensures the vehicle's safety when another car overtakes it while stationary.
The vehicle waits until the maneuver is complete before departing.

## Scenario Outline: Overtake from left or right side

Tests the case where a following vehicle overtakes from either the left or the right.Verifies the vehicle correctly detects the completed overtake and departs at a safe moment.

* Given Ego is in standstill
* And Car0 is positioned behind Ego, in the same driving lane
* And Car0 is driving at <vx_car0> towards Ego
* When Car0 reaches a position at distance <dr> behind Ego
* And Car0 starts overtaking Ego
* Then Ego remains standstill
* When Car0 reaches a position <df> ahead of Ego
* And Car0 finishes the overtake maneuver
* Then Ego starts driving
* And Ego reaches the destination without collision
* And Ego does not decelerate at a rate below <amin_ego> during driving

![Overview](./images/UC-NTR-001-0001.drawio.svg)

### Examples: Left side overtake

  | vx_car0 | dr    | df    | overtake_offset | amin_ego   |
  | ------- | ----- | ----- | --------------- | ---------- |
  | 20 km/h | 7.0 m | 5.0 m | 2.5 m           | -1.5 m/s^2 |
  | 30 km/h | 7.0 m | 5.0 m | 2.5 m           | -1.5 m/s^2 |
  | 40 km/h | 7.0 m | 5.0 m | 2.5 m           | -1.5 m/s^2 |
  
### Examples: Right side overtake

  | vx_car0 | dr    | df    | overtake_offset | amin_ego   |
  | ------- | ----- | ----- | --------------- | ---------- |
  | 20 km/h | 7.0 m | 5.0 m | -2.5 m           | -1.5 m/s^2 |
  | 30 km/h | 7.0 m | 5.0 m | -2.5 m           | -1.5 m/s^2 |
  | 40 km/h | 7.0 m | 5.0 m | -2.5 m           | -1.5 m/s^2 |
  